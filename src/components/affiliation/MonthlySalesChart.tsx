...
export default function MonthlySalesChart() {
  const [series, setSeries] = useState([{ name: "Affiliés", data: Array(12).fill(0) }]);
  ...
  useEffect(() => {
    const fetchAffiliations = async () => {
      ...
      data?.forEach((item) => {
        const month = new Date(item.created_at).getMonth(); // 0 = Janvier
        monthlyCounts[month]++;
      });

      setSeries([{ name: "Affiliés", data: monthlyCounts }]);
    };
  ...
  const options: ApexOptions = {
    ...
    tooltip: {
      x: { show: false },
      y: { formatter: (val: number) => `${val} affiliés` },
    },
  };
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Affiliés / Mois</h3>
